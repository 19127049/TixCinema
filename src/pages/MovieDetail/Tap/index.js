...
import scroll from "../../../util/settings/scroll";
...
import {
  ...
  likeComment,
  postComment,
} from "../../../redux/actions/MovieDetail";
moment.locale("vi");
...
export default function CenteredTabs({
  ...
  const [dataComment, setdataComment] = useState({
    avtId: thongTinNguoiDung?.taiKhoan,
    username: thongTinNguoiDung?.hoTen,
    point: 2.5,
    post: "",
    likes: 0,
    maPhim: param.maPhim,
  ...
  const handleLike = (id) => {
    ...
    if (
      commentUserLiked.userLikeThisComment.includes(thongTinNguoiDung.email)
    ) {
      ...
      commentUserLiked.userLikeThisComment =
        commentUserLiked.userLikeThisComment.filter((item) => {
          return item !== thongTinNguoiDung.email;
        });
      commentUserLiked.likes = commentUserLiked.likes - 1;
    } else {
      commentUserLiked.userLikeThisComment.push(thongTinNguoiDung.email);
      commentUserLiked.likes = commentUserLiked.likes + 1;
    }
            ...
            <div className="col-sm-6 col-xs-12">
              <div className="row mb-2">
                ...
                <p className={`float-left ${classes.contentInfo}`}>
                  {formatDate(data.content?.ngayKhoiChieu?.slice(0, 10)).YyMmDd}
                </p>
              </div>
              <div className="row mb-2">
          ...
          <div className={classes.danhGia}>
            <div className={classes.inputRoot} onClick={handleClickComment}>
              <span className={classes.avatarReviewer}>
                {/* <img
                  src={thongTinNguoiDung ? FAKE_AVATAR : UNKNOW_USER}
                  alt="avatar"
